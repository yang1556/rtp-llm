...
protected:
    ...
    static std::shared_ptr<TransferTask> makeTask(int64_t cache_key,
                                                   void*   addr,
                                                   size_t  size_bytes,
                                                   int64_t deadline_offset_ms = 5000) {
        ...
        return std::make_shared<TransferTask>(std::move(block_infos),
                                              currentTimeMs() + deadline_offset_ms);
    }

    // ----- Context builder -----

    std::unique_ptr<TcpTaskContext> makeContext(
