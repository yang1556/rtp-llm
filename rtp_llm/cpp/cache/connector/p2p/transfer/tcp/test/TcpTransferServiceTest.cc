...
public:
    void Run() override { p_.set_value(); }
...
static ::tcp_transfer::TcpLayerBlockTransferRequest makeRequest(const std::string& unique_key,
                                                                 int64_t            deadline_ms) {
...
static KeyBlockInfoMap makeTwoSubBlocks(int64_t cache_key, void* addr0, size_t sz0,
                                         void* addr1, size_t sz1) {
    auto kbi = std::make_shared<KeyBlockInfo>();
...
protected:
    ...
    std::unique_ptr<BlockingClosure> issueTransfer(
            const ::tcp_transfer::TcpLayerBlockTransferRequest* req,
...
TEST_F(TcpTransferServiceTest, A1_NormalTransfer_RecvFirst_Success) {
    if (!device_) { GTEST_SKIP() << "No GPU device"; }

    constexpr size_t    size    = 64;
    const std::string   content(size, 'A');
    auto                gpu_buf = allocDevice(size);

    auto task = task_store_->addTask("k1", makeBlocks(1, gpu_buf->data(), size),
                                     currentTimeMs() + 5000);
...
TEST_F(TcpTransferServiceTest, A2_NormalTransfer_TransferFirst_RecvLater_Success) {
    ...
    auto task = task_store_->addTask("k2", makeBlocks(1, gpu_buf->data(), size),
                                     currentTimeMs() + 5000);
    ASSERT_NE(task, nullptr);
...
TEST_F(TcpTransferServiceTest, B3_ContextTimeout_OverridesErrorOnNotifyDone) {
    ...
    auto task = task_store_->addTask("k_b3",
                                     makeBlocks(1, dummy, 64),
...
TEST_F(TcpTransferServiceTest, C2_CancelDuringTransferring_ReturnsCancelled) {
    ...
    auto task = task_store_->addTask("k_c2", makeBlocks(1, dummy, 64),
                                     currentTimeMs() + 5000);
    ASSERT_NE(task, nullptr);
...
TEST_F(TcpTransferServiceTest, E1_BufferMismatch_SizeMismatch) {
    char dummy[64]{};
    auto task = task_store_->addTask("k_e1", makeBlocks(1, dummy, 64),
                                     currentTimeMs() + 5000);
    ASSERT_NE(task, nullptr);
...
TEST_F(TcpTransferServiceTest, E2_BufferMismatch_ReceiverKeyMissingInSender) {
    char dummy[64]{};
    auto task = task_store_->addTask("k_e2", makeBlocks(1, dummy, 64),
                                     currentTimeMs() + 5000);
    ASSERT_NE(task, nullptr);
...
TEST_F(TcpTransferServiceTest, E2b_ExtraKeyInSender_IgnoredOnSuccess) {
    ...
    auto task = task_store_->addTask("k_e2b", makeBlocks(1, gpu_buf->data(), size),
                                     currentTimeMs() + 5000);
    ASSERT_NE(task, nullptr);
...
TEST_F(TcpTransferServiceTest, E3_BufferMismatch_SubBlockOutOfRange) {
    ...
    auto task = task_store_->addTask("k_e3",
                                     makeTwoSubBlocks(1, dummy, 64, dummy + 64, 64),
                                     currentTimeMs() + 5000);
    ASSERT_NE(task, nullptr);
...
TEST_F(TcpTransferServiceTest, G3_ConcurrentTransfers_SameKey_BothComplete) {
    ...
    auto task = task_store_->addTask("k_g3", makeBlocks(1, gpu_buf->data(), size),
                                     currentTimeMs() + 5000);
    ASSERT_NE(task, nullptr);
