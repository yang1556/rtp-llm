...
AsymmetricTpUtil::AsymmetricTpUtil(int64_t tp_size, int64_t tp_rank):
    tp_size_(tp_size), tp_rank_(tp_rank) {}
...
AsymmetricTpUtil::handleAsymmetricTP(const std::vector<std::pair<std::string, uint32_t>>& decode_transfer_servers) {
    if (tp_size_ > static_cast<int64_t>(decode_transfer_servers.size())) {
...
AsymmetricTpUtil::handleNP1D(const std::vector<std::pair<std::string, uint32_t>>& decode_transfer_servers) {
    if (tp_size_ % static_cast<int64_t>(decode_transfer_servers.size()) != 0) {
        RTP_LLM_LOG_ERROR(
    ...
    auto& decode_transfer_server =
        decode_transfer_servers[static_cast<size_t>(tp_rank_ / remote_partition_count)];
...
AsymmetricTpUtil::handleND1P(const std::vector<std::pair<std::string, uint32_t>>& decode_transfer_servers) {
    if (decode_transfer_servers.size() % static_cast<size_t>(tp_size_) != 0) {
        RTP_LLM_LOG_ERROR(
    ...
    auto local_partition_count  = static_cast<int>(decode_transfer_servers.size() / tp_size_);
    for (int i = 0; i < local_partition_count; i++) {
        auto decode_transfer_server =
            decode_transfer_servers[static_cast<size_t>(tp_rank_ * local_partition_count + i)];
