...
template<int N>
__host__ __device__ __forceinline__ Arr<float, N> operator*(const Arr<float, N>& a, const Arr<float, N>& b) {
    Arr<float, N> o;
#pragma unroll
    for (int i = 0; i < N; ++i)
        o[i] = a[i] * b[i];
...
template<int N>
__host__ __device__ __forceinline__ Arr<float, N> operator*(const Arr<float, N>& a, float s) {
    Arr<float, N> o;
#pragma unroll
    for (int i = 0; i < N; ++i)
        o[i] = a[i] * s;
...
__host__ __device__ __forceinline__ U arrayConvert(T const& in) {
    static_assert(T::kElements == U::kElements, "kElements mismatch");
    using DType = typename U::Element;
    U u;
#pragma unroll
    for (int i = 0; i < U::kElements; ++i)
        u[i] = static_cast<DType>(in[i]);
    return u;
}

template<int N>
__host__ __device__ __forceinline__ float max_abs(Arr<float, N> const& a) {
    float m = 0.f;
#pragma unroll
    for (int i = 0; i < N; ++i) {
        float av = fabsf(a[i]);
...
// ROCm's __hip_fp8_e4m3_fnuz(float) and default ctor are __host__ only; use manual encode + reinterpret for device.
__device__ __forceinline__ fp8_e4m3_t to_e4m3fnuz(float x) {
    float   c   = fminf(fmaxf(x, -FP8_E4M3_MAX), FP8_E4M3_MAX);
    uint8_t raw = fp8_e4m3fnuz_encode<Fp8RoundMode::RNE>(c);
    return *reinterpret_cast<fp8_e4m3_t*>(&raw);
}

template<int N>
__device__ __forceinline__ Arr<fp8_e4m3_t, N> pack_fp8_scaled(const Arr<float, N>& a, float inv_scale) {
    // No fp8_e4m3_t/Arr construction (host-only on ROCm); use byte buffer then reinterpret.
    uint8_t bytes[N];
#pragma unroll
    for (int i = 0; i < N; ++i) {
        bytes[i] = fp8_e4m3fnuz_encode<Fp8RoundMode::RNE>(a[i] * inv_scale);
    }
    return *reinterpret_cast<Arr<fp8_e4m3_t, N>*>(bytes);
}
