...
typedef __hip_bfloat162       hip_bfloat162;
...
__device__ void fast_dequant_f8f16x4(uint32_t* input, uint2* output) {
    ...
    if constexpr (std::is_same_v<fp8_dtype, __nv_fp8_e5m2> && std::is_same_v<fp16_dtype, half>) {
        ...
        // Calculate MASK_FLASHINFER for extracting mantissa and exponent
        constexpr int MASK1_FLASHINFER = 0x80000000;
        constexpr int MASK2_FLASHINFER = MASK1_FLASHINFER >> (FP8_EXPONENT + FP8_MANTISSA);
        constexpr int MASK3_FLASHINFER = MASK2_FLASHINFER & 0x7fffffff;
        constexpr int MASK_FLASHINFER  = MASK3_FLASHINFER | (MASK3_FLASHINFER >> 16);
        q                              = __byte_perm(q, q, 0x1302);

        // Extract and shift FP8 values to FP16 format
        uint32_t Out1 = (q & 0x80008000) | ((q & MASK_FLASHINFER) >> RIGHT_SHIFT);
        uint32_t Out2 = ((q << 8) & 0x80008000) | (((q << 8) & MASK_FLASHINFER) >> RIGHT_SHIFT);
...
FLASHINFER_INLINE void cast_from_impl(vec_t<tgt_float_t, vec_size>& dst, const vec_t<src_float_t, vec_size>& src) {
    ...
    auto src_ptr = const_cast<vec_t<src_float_t, vec_size>*>(&src)->ptr();
    auto dst_ptr = const_cast<vec_t<tgt_float_t, vec_size>*>(&dst)->ptr();

    if constexpr (std::is_same_v<src_float_t, float> && std::is_same_v<tgt_float_t, hip_bfloat16>) {
        for (size_t i = 0; i < vec_size; ++i) {
            dst[i] = (__float2bfloat16(src_ptr[i]));
        }
    } else if (std::is_same_v<src_float_t, hip_bfloat16> && std::is_same_v<tgt_float_t, float>) {
        __hip_bfloat16* data_src = reinterpret_cast<__hip_bfloat16*>(src_ptr);
        float*          data_tgt = reinterpret_cast<float*>(dst_ptr);
        for (size_t i = 0; i < vec_size; ++i) {
            data_tgt[i] = __bfloat162float(data_src[i]);
        }
    } else {
        if constexpr (std::is_same<tgt_float_t, float>::value) {
            printf("tgt_float_t is float\n");
        } else if constexpr (std::is_same<tgt_float_t, __half>::value) {
            printf("tgt_float_t is half\n");
        } else if constexpr (std::is_same<tgt_float_t, hip_bfloat16>::value) {
            printf("tgt_float_t is hip_bfloat16\n");
        } else {
            printf("tgt_float_t is unknown\n");
        }

        if constexpr (std::is_same<src_float_t, float>::value) {
            printf("src_float_t is float\n");
        } else if constexpr (std::is_same<src_float_t, __half>::value) {
            printf("src_float_t is half\n");
        } else if constexpr (std::is_same<src_float_t, hip_bfloat16>::value) {
            printf("src_float_t is hip_bfloat16\n");
        } else {
            printf("src_float_t is unknown\n");
        }
        printf("%s: %d can not support convert\n", __FILE__, __LINE__);
    }
}
