...
#pragma unroll
    ...
    __device__ __forceinline__ static bool check_and_reset(__nv_fp8_e4m3* val_ptr) {
        uint8_t bits = *reinterpret_cast<uint8_t*>(val_ptr);
        if ((bits & 0x7Fu) == 0x7Fu) {
...
__global__ void check_and_reset_kv_cache_prefill_kernel(const void* const* __restrict__ layer_base_addr,
    ...
    const size_t group_id = layer_to_group ? static_cast<size_t>(layer_to_group[layer_id]) : 0;
    if (group_types && group_types[group_id] == 0)  // LINEAR: skip check
        return;
    ...
    const int32_t* batch_blocks = kv_cache_block_id + group_id * (batch_dim * max_blocks_per_batch)
                                  + (batch_start + batch_id) * max_blocks_per_batch;
...
__global__ void check_and_reset_kv_cache_decode_kernel(const void* const* __restrict__ layer_base_addr,
    ...
    char*        base_ptr = static_cast<char*>(const_cast<void*>(base_ptr_void));
    const size_t seq_len  = sequence_lengths[batch_id];

    if (seq_len == 0)
        return;

    const size_t last_token_idx    = seq_len - 1;
    const size_t logical_block_idx = last_token_idx / seq_size_per_block;
    const size_t offset_in_block   = last_token_idx % seq_size_per_block;
    ...
    const int32_t* batch_blocks = kv_cache_block_id + group_id * (batch_dim * max_blocks_per_batch)
                                  + (batch_start + batch_id) * max_blocks_per_batch;
    const int32_t physical_block_id = batch_blocks[logical_block_idx];
    ...
    const size_t num_warps = (blockDim.x + 31) / 32;

    bool thread_has_nan = false;

    const size_t heads_per_warp = (local_head_num_kv + num_warps - 1) / num_warps;
    const size_t warp_start     = warp_id * heads_per_warp;
    const size_t warp_end       = min(warp_start + heads_per_warp, static_cast<size_t>(local_head_num_kv));

    for (size_t head_idx = warp_start + lane_id; head_idx < warp_end; head_idx += 32) {
        char* k_data = block_base + head_idx * (seq_size_per_block * k_token_bytes) + offset_in_block * k_token_bytes;

        const size_t vec4_count = k_token_bytes >> 4;
...
#pragma unroll 4
        ...
        const size_t remainder = k_token_bytes & 15;
        if (remainder) {
            T*           remainder_ptr   = reinterpret_cast<T*>(k_data + (vec4_count << 4));
            const size_t remainder_elems = remainder / sizeof(T);
    ...
    for (size_t head_idx = warp_start + lane_id; head_idx < warp_end; head_idx += 32) {
        ...
        const size_t vec4_count = v_token_bytes >> 4;
...
#pragma unroll 4
        ...
        const size_t remainder = v_token_bytes & 15;
        if (remainder) {
