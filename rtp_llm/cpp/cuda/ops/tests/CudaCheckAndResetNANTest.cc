...
protected:
    ...
    testKVCachePrefillCheckAndResetNAN(size_t                      batch_size,
        ...
        err = cudaMalloc(&d_kv_cache_block_id, batch_size * max_blocks_per_batch * sizeof(float));
        ...
        err = cudaMalloc(&d_prefix_lengths, batch_size * sizeof(float));
        ...
        err = cudaMalloc(&d_seq_len_cu, batch_size * sizeof(float));
        ASSERT_EQ(err, cudaSuccess) << "Failed to allocate d_seq_len_cu: " << cudaGetErrorString(err);
        cudaMemcpy(d_seq_len_cu, seq_len_cu.data(), batch_size * sizeof(float), cudaMemcpyHostToDevice);
        ASSERT_EQ(cudaGetLastError(), cudaSuccess);

        // Allocate nan_flag
        float* d_nan_flag;
        err = cudaMalloc(&d_nan_flag, batch_size * sizeof(float));
    ...
    testKVCacheDecodeCheckAndResetNAN(size_t                      batch_size,
        ...
        err = cudaMalloc(&d_sequence_lengths, batch_size * sizeof(float));
        ASSERT_EQ(err, cudaSuccess) << "Failed to allocate d_sequence_lengths: " << cudaGetErrorString(err);
        cudaMemcpy(d_sequence_lengths, sequence_lengths.data(), batch_size * sizeof(float), cudaMemcpyHostToDevice);
        ASSERT_EQ(cudaGetLastError(), cudaSuccess);
...
// Regression: prefill kernel must check/rewrite remainder bytes when token bytes are not 16-byte aligned.
// Use FP8 with odd head dim (127) so k_token_bytes/v_token_bytes = 127 and tail byte is outside uint4 loop.
TEST_F(CudaCheckAndResetNANTest, TestFP8_KVCachePrefill_RemainderTailDetection) {
    const size_t batch_size           = 1;
    const size_t layer_num            = 1;
    const size_t max_blocks_per_batch = 1;
    const size_t seq_size_per_block   = 8;
    const size_t local_head_num_kv    = 1;
    const size_t k_token_size         = 127;  // non-16 aligned bytes for FP8
    const size_t v_token_size         = 127;
    const size_t element_size         = sizeof(__nv_fp8_e4m3);  // 1 byte
    const size_t k_token_bytes        = k_token_size * element_size;
    const size_t v_token_bytes        = v_token_size * element_size;
    const size_t k_block_size_bytes   = local_head_num_kv * k_token_size * seq_size_per_block * element_size;
    const size_t v_block_size_bytes   = local_head_num_kv * v_token_size * seq_size_per_block * element_size;
    const size_t block_size_bytes     = k_block_size_bytes + v_block_size_bytes;

    const std::vector<int32_t> prefix_lengths = {0};
    const std::vector<int32_t> input_lengths  = {1};  // only token 0 is checked
    const std::vector<int32_t> h_block_ids    = {0};

    const size_t kv_cache_bytes = layer_num * block_size_bytes;
    __nv_fp8_e4m3* d_kv_cache   = nullptr;
    ASSERT_EQ(cudaMalloc(&d_kv_cache, kv_cache_bytes), cudaSuccess);
    std::vector<uint8_t> h_init(kv_cache_bytes, 0x01u);
    cudaMemcpy(d_kv_cache, h_init.data(), kv_cache_bytes, cudaMemcpyHostToDevice);

    std::vector<void*> h_layer_base(layer_num);
    h_layer_base[0] = reinterpret_cast<void*>(d_kv_cache);
    void** d_layer_base = nullptr;
    ASSERT_EQ(cudaMalloc(&d_layer_base, layer_num * sizeof(void*)), cudaSuccess);
    cudaMemcpy(d_layer_base, h_layer_base.data(), layer_num * sizeof(void*), cudaMemcpyHostToDevice);

    int32_t* d_block_ids = nullptr;
    ASSERT_EQ(cudaMalloc(&d_block_ids, h_block_ids.size() * sizeof(float)), cudaSuccess);
    cudaMemcpy(d_block_ids, h_block_ids.data(), h_block_ids.size() * sizeof(int32_t), cudaMemcpyHostToDevice);

    int32_t* d_prefix_lengths = nullptr;
    int32_t* d_input_lengths  = nullptr;
    ...
    uint8_t* k_tail_ptr = reinterpret_cast<uint8_t*>(d_kv_cache) + (k_token_bytes - 1);
    uint8_t  nan_raw    = 0x7Fu;  // FP8 E4M3 NaN
    cudaMemcpy(k_tail_ptr, &nan_raw, sizeof(uint8_t), cudaMemcpyHostToDevice);

    invokeCheckAndResetNANKvCachePrefill<__nv_fp8_e4m3>(reinterpret_cast<const void* const*>(d_layer_base),
                                                        d_block_ids,
                                                        d_prefix_lengths,
                                                        d_input_lengths,
                                                        batch_size,
    ...
    EXPECT_EQ(tail_after, 0u) << "NaN in FP8 remainder byte should be reset to 0 in prefill";

    cudaFree(d_kv_cache);
    cudaFree(d_layer_base);
...
// Regression: (raw>>3)&0xF==0xF used to reset all exp=0xF; only (raw&0x7F)==0x7F (NaN 0x7F/0xFF) should be reset.
// Exp=0xF, mantissa 0..6 (raw 0x78..0x7E, 0xF8..0xFE) correspond to ±240, ±256, ±288, ±320, ±352, ±384, ±448
// and must NOT be reset. NaN (0x7F/0xFF) must be reset.
TEST_F(CudaCheckAndResetNANTest, TestFP8_ExpFNotNan_NotReset_And_NaN_Reset) {
    size_t batch_size           = 1;
    size_t layer_num            = 2;
    size_t seq_size_per_block   = 16;
    size_t max_blocks_per_batch = 2;
    ...
    size_t layer_size            = max_blocks_per_batch * block_size_bytes;
    size_t kv_cache_buffer_size  = layer_num * layer_size;
    size_t token_bytes           = block_size_bytes / seq_size_per_block;
    size_t k_token_size_per_head = token_bytes / (2 * element_size);
    size_t v_token_size_per_head = k_token_size_per_head;
    size_t k_block_size_bytes    = k_token_size_per_head * seq_size_per_block * element_size;

    std::vector<int32_t>              prefix_lengths    = {0};
    std::vector<int32_t>              seq_len_cu        = {20};
    std::vector<std::vector<int32_t>> kv_cache_block_id = {{0, 1}};

    __nv_fp8_e4m3* d_kv_cache = nullptr;
    ASSERT_EQ(cudaMalloc(&d_kv_cache, kv_cache_buffer_size), cudaSuccess);
    std::vector<__nv_fp8_e4m3> h_init(kv_cache_buffer_size / element_size, __nv_fp8_e4m3(1.0f));
    cudaMemcpy(d_kv_cache, h_init.data(), kv_cache_buffer_size, cudaMemcpyHostToDevice);

    void** d_layer_base_addr = nullptr;
    ASSERT_EQ(cudaMalloc(&d_layer_base_addr, layer_num * sizeof(void*)), cudaSuccess);
    std::vector<void*> h_layer_base_addr(layer_num);
    for (size_t i = 0; i < layer_num; i++) {
        h_layer_base_addr[i] = reinterpret_cast<char*>(d_kv_cache) + i * layer_size;
    }
    cudaMemcpy(d_layer_base_addr, h_layer_base_addr.data(), layer_num * sizeof(void*), cudaMemcpyHostToDevice);

    std::vector<int32_t> h_kv_cache_block_id(batch_size * max_blocks_per_batch, -1);
    for (size_t b = 0; b < batch_size; b++) {
        for (size_t j = 0; j < kv_cache_block_id[b].size(); j++) {
            h_kv_cache_block_id[b * max_blocks_per_batch + j] = kv_cache_block_id[b][j];
        }
    }
    int32_t* d_kv_cache_block_id = nullptr;
    ASSERT_EQ(cudaMalloc(&d_kv_cache_block_id, batch_size * max_blocks_per_batch * sizeof(float)), cudaSuccess);
    cudaMemcpy(d_kv_cache_block_id,
               h_kv_cache_block_id.data(),
               batch_size * max_blocks_per_batch * sizeof(int32_t),
               cudaMemcpyHostToDevice);
    ...
    ASSERT_EQ(cudaMalloc(&d_prefix_lengths, batch_size * sizeof(float)), cudaSuccess);
    cudaMemcpy(d_prefix_lengths, prefix_lengths.data(), batch_size * sizeof(float), cudaMemcpyHostToDevice);
    int32_t* d_seq_len_cu = nullptr;
    ASSERT_EQ(cudaMalloc(&d_seq_len_cu, batch_size * sizeof(float)), cudaSuccess);
    cudaMemcpy(d_seq_len_cu, seq_len_cu.data(), batch_size * sizeof(float), cudaMemcpyHostToDevice);
    float* d_nan_flag = nullptr;
    ASSERT_EQ(cudaMalloc(&d_nan_flag, batch_size * sizeof(float)), cudaSuccess);

    // Raw bytes that old bug (raw>>3)&0xF==0xF would wrongly reset; (raw&0x7F)!=0x7F so must NOT be reset.
    // 0x78..0x7E -> +256,+288,+320,+352,+384,+416,+448; 0xF8..0xFE -> -256,-288,-320,-352,-384,-416,-448 (and ±240 in
    // range).
    const std::vector<uint8_t> must_not_reset_raw = {
        0x78, 0x79, 0x7A, 0x7B, 0x7C, 0x7D, 0x7E, 0xF8, 0xF9, 0xFA, 0xFB, 0xFC, 0xFD, 0xFE};
    const size_t token_stride_bytes = k_token_size_per_head * element_size;
    // Layer 0, block 0, head 0: token i at offset i * token_stride_bytes from layer0_block0_base.
    size_t layer0_block0_offset = 0 * layer_size + 0 * block_size_bytes;

    for (size_t i = 0; i < must_not_reset_raw.size(); i++) {
        size_t               offset = layer0_block0_offset + i * token_stride_bytes;
        std::vector<uint8_t> fill(16, must_not_reset_raw[i]);
        cudaMemcpy(reinterpret_cast<char*>(d_kv_cache) + offset, fill.data(), 16, cudaMemcpyHostToDevice);
    }
    // Token 14: 0x7F (NaN) -> must be reset to 0
    size_t               offset_7f = layer0_block0_offset + 14 * token_stride_bytes;
    std::vector<uint8_t> fill_7f(16, 0x7F);
    cudaMemcpy(reinterpret_cast<char*>(d_kv_cache) + offset_7f, fill_7f.data(), 16, cudaMemcpyHostToDevice);

    cudaMemset(d_nan_flag, 0, batch_size * sizeof(float));
    invokeCheckAndResetNANKvCachePrefill<__nv_fp8_e4m3>(reinterpret_cast<const void* const*>(d_layer_base_addr),
                                                        d_kv_cache_block_id,
                                                        d_prefix_lengths,
                                                        d_seq_len_cu,
                                                        batch_size,
                                                        ...
                                                        0u,          // batch_start
                                                        max_blocks_per_batch,
                                                        1u,  // local_head_num_kv
                                                        k_token_size_per_head,
                                                        v_token_size_per_head,
                                                        k_block_size_bytes,
                                                        k_block_size_bytes,
                                                        k_token_size_per_head * element_size,
                                                        v_token_size_per_head * element_size,
                                                        block_size_bytes,
                                                        seq_size_per_block,
                                                        d_nan_flag,
                                                        stream_);
    ...
    for (size_t i = 0; i < must_not_reset_raw.size(); i++) {
        size_t  offset    = layer0_block0_offset + i * token_stride_bytes;
        uint8_t read_back = 0;
        cudaMemcpy(&read_back, reinterpret_cast<char*>(d_kv_cache) + offset, 1, cudaMemcpyDeviceToHost);
        ASSERT_EQ(read_back, must_not_reset_raw[i])
            << "raw 0x" << std::hex << (unsigned)must_not_reset_raw[i] << " (exp=0xF, not NaN) must NOT be reset";
    }
    uint8_t read_back = 0;
    cudaMemcpy(&read_back, reinterpret_cast<char*>(d_kv_cache) + offset_7f, 1, cudaMemcpyDeviceToHost);
    ASSERT_EQ(read_back, 0u) << "0x7F (NaN) must be reset to 0";

    // Decode: 0xF8 at last token must NOT be reset
    cudaMemcpy(d_kv_cache, h_init.data(), kv_cache_buffer_size, cudaMemcpyHostToDevice);
    std::vector<int32_t> sequence_lengths    = {20};
    size_t               num_physical_blocks = 2;
    size_t               offset_decode_f8    = 0 * (num_physical_blocks * block_size_bytes) + 1 * block_size_bytes
                              + 0 * (seq_size_per_block * token_stride_bytes) + 3 * token_stride_bytes;
    std::vector<uint8_t> fill_f8(16, 0xF8);
    cudaMemcpy(reinterpret_cast<char*>(d_kv_cache) + offset_decode_f8, fill_f8.data(), 16, cudaMemcpyHostToDevice);

    for (size_t i = 0; i < layer_num; i++) {
        h_layer_base_addr[i] = reinterpret_cast<char*>(d_kv_cache) + i * num_physical_blocks * block_size_bytes;
    }
    cudaMemcpy(d_layer_base_addr, h_layer_base_addr.data(), layer_num * sizeof(void*), cudaMemcpyHostToDevice);
    int32_t* d_sequence_lengths = nullptr;
    ASSERT_EQ(cudaMalloc(&d_sequence_lengths, batch_size * sizeof(float)), cudaSuccess);
    cudaMemcpy(d_sequence_lengths, sequence_lengths.data(), batch_size * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemset(d_nan_flag, 0, batch_size * sizeof(float));

    invokeCheckAndResetNANKvCacheDecode<__nv_fp8_e4m3>(reinterpret_cast<const void* const*>(d_layer_base_addr),
                                                       d_kv_cache_block_id,
                                                       d_sequence_lengths,
                                                       batch_size,
                                                       layer_num,
    ...
    cudaStreamSynchronize(stream_);
    ASSERT_EQ(cudaGetLastError(), cudaSuccess);
    cudaMemcpy(&read_back, reinterpret_cast<char*>(d_kv_cache) + offset_decode_f8, 1, cudaMemcpyDeviceToHost);
    ASSERT_EQ(read_back, 0xF8u) << "0xF8 at last token must NOT be reset";
...
TEST_F(CudaCheckAndResetNANTest, TestFP8_KVCacheDecode_Basic) {
    ...
    // Only NaN (0x7F/0xFF) is reset per NVIDIA FP8 E4M3 spec; Inf (0x78/0xF8) is not.
    std::vector<std::tuple<size_t, size_t, int32_t, bool, bool>> positions = {
        {0, 0, 19, false, true},  // NaN at last token -> reset
    };
...
// Regression test: decode kernel multi-warp NaN detection.
// When local_head_num_kv > 32, the kernel launches multiple warps.
// NaN in heads handled by warp 1+ must still be detected.
// This test places NaN only in head 48 (warp 1's territory) and verifies nan_flag is set.
TEST_F(CudaCheckAndResetNANTest, TestFloat_KVCacheDecode_MultiWarpNanDetection) {
    // Use 64 KV heads so kernel launches 2 warps (warp 0: heads 0-31, warp 1: heads 32-63).
    const size_t batch_size           = 2;
    const size_t layer_num            = 2;
    const size_t seq_size_per_block   = 8;
    const size_t max_blocks_per_batch = 2;
    const size_t local_head_num_kv    = 64;  // triggers 2 warps
    const size_t size_per_head        = 16;  // 16 floats = 64 bytes, multiple of 16 for uint4
    const size_t element_size         = sizeof(float);

    // MHA block layout: [K: local_head_num_kv * seq_size_per_block * size_per_head] [V: same]
    const size_t k_token_size_per_head = size_per_head;
    const size_t v_token_size_per_head = size_per_head;
    const size_t k_block_size_bytes = local_head_num_kv * k_token_size_per_head * seq_size_per_block * element_size;
    const size_t v_block_size_bytes = local_head_num_kv * v_token_size_per_head * seq_size_per_block * element_size;
    const size_t block_size_bytes   = k_block_size_bytes + v_block_size_bytes;
    const size_t k_token_bytes      = k_token_size_per_head * element_size;
    const size_t v_token_bytes      = v_token_size_per_head * element_size;

    // batch 0: seq_len=10, last token idx=9 (block 1, offset 1)
    // batch 1: seq_len=5,  last token idx=4 (block 0, offset 4)
    std::vector<int32_t> sequence_lengths = {10, 5};

    const size_t num_physical_blocks  = 4;
    const size_t layer_size           = num_physical_blocks * block_size_bytes;
    const size_t kv_cache_buffer_size = layer_num * layer_size;

    // Allocate and init KV cache
    float* d_kv_cache = nullptr;
    ASSERT_EQ(cudaMalloc(&d_kv_cache, kv_cache_buffer_size), cudaSuccess);
    std::vector<float> h_init(kv_cache_buffer_size / element_size, 1.0f);
    cudaMemcpy(d_kv_cache, h_init.data(), kv_cache_buffer_size, cudaMemcpyHostToDevice);

    // Layer base addresses
    std::vector<void*> h_layer_base(layer_num);
    for (size_t l = 0; l < layer_num; l++) {
        h_layer_base[l] = reinterpret_cast<char*>(d_kv_cache) + l * layer_size;
    }
    void** d_layer_base = nullptr;
    ASSERT_EQ(cudaMalloc(&d_layer_base, layer_num * sizeof(void*)), cudaSuccess);
    cudaMemcpy(d_layer_base, h_layer_base.data(), layer_num * sizeof(void*), cudaMemcpyHostToDevice);

    // Block IDs: batch 0 → blocks {0,1}, batch 1 → blocks {2,3}
    std::vector<int32_t> h_block_ids = {0, 1, 2, 3};
    int32_t* d_block_ids = nullptr;
    ASSERT_EQ(cudaMalloc(&d_block_ids, h_block_ids.size() * sizeof(float)), cudaSuccess);
    cudaMemcpy(d_block_ids, h_block_ids.data(), h_block_ids.size() * sizeof(int32_t), cudaMemcpyHostToDevice);
    ...
    // --- Insert NaN only in head 48 (warp 1's territory) for batch 0, layer 0 ---
    // last token for batch 0: idx=9, block=1, offset_in_block=1
    // K data for head 48: block_base + head_48 * (seq_size_per_block * k_token_bytes) + offset * k_token_bytes
    {
        const size_t target_head       = 48;  // warp 1 handles this head
        const size_t physical_block    = 1;   // batch 0's second block
        const size_t offset_in_block   = 1;   // token 9 % 8 = 1
        const size_t layer_id          = 0;

        char* block_base = reinterpret_cast<char*>(h_layer_base[layer_id])
                           + physical_block * block_size_bytes;
        char* k_data = block_base
                       + target_head * (seq_size_per_block * k_token_bytes)
                       + offset_in_block * k_token_bytes;

        // Write NaN values (16 floats = one full head token)
        std::vector<float> nan_data(size_per_head, std::nanf(""));
        cudaMemcpy(k_data, nan_data.data(), k_token_bytes, cudaMemcpyHostToDevice);
    }

    // --- Insert NaN only in head 60 (warp 1) for batch 1, layer 1, V part ---
    {
        const size_t target_head       = 60;
        const size_t physical_block    = 2;   // batch 1's first block
        const size_t offset_in_block   = 4;   // token 4 % 8 = 4
        const size_t layer_id          = 1;

        char* block_base = reinterpret_cast<char*>(h_layer_base[layer_id])
                           + physical_block * block_size_bytes;
        // V part starts after K part
        char* v_data = block_base + k_block_size_bytes
                       + target_head * (seq_size_per_block * v_token_bytes)
                       + offset_in_block * v_token_bytes;

        std::vector<float> nan_data(size_per_head, std::nanf(""));
        cudaMemcpy(v_data, nan_data.data(), v_token_bytes, cudaMemcpyHostToDevice);
    }

    cudaStreamSynchronize(stream_);

    // Run decode kernel
    invokeCheckAndResetNANKvCacheDecode<float>(
        reinterpret_cast<const void* const*>(d_layer_base),
        d_block_ids,
        d_seq_lens,
        batch_size,
        layer_num,
    ...
    {
        const size_t target_head     = 48;
        const size_t physical_block  = 1;
        const size_t offset_in_block = 1;
        char* block_base = reinterpret_cast<char*>(h_layer_base[0]) + physical_block * block_size_bytes;
        char* k_data = block_base + target_head * (seq_size_per_block * k_token_bytes) + offset_in_block * k_token_bytes;

        std::vector<float> h_readback(size_per_head);
        cudaMemcpy(h_readback.data(), k_data, k_token_bytes, cudaMemcpyDeviceToHost);
        for (size_t i = 0; i < size_per_head; i++) {
            EXPECT_EQ(h_readback[i], 0.0f) << "Batch 0 head 48 K[" << i << "] not reset to 0";
        }
    }
