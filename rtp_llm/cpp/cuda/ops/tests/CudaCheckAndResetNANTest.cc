...
TEST_F(CudaCheckAndResetNANTest, TestFloat_KVCacheDecode_MultiWarpNanDetection) {
    ...
    // K data for head 48: block_base + head_48 * (seq_size_per_block * k_token_bytes) + offset * k_token_bytes
    {
        ...
        char* block_base = reinterpret_cast<char*>(h_layer_base[layer_id]) + physical_block * block_size_bytes;
        char* k_data =
            block_base + target_head * (seq_size_per_block * k_token_bytes) + offset_in_block * k_token_bytes;
    ...
    // --- Insert NaN only in head 60 (warp 1) for batch 1, layer 1, V part ---
    {
        ...
        char* v_data = block_base + k_block_size_bytes + target_head * (seq_size_per_block * v_token_bytes)
    ...
    // Run decode kernel
    invokeCheckAndResetNANKvCacheDecode<float>(reinterpret_cast<const void* const*>(d_layer_base),
                                               d_block_ids,
                                               d_seq_lens,
                                               batch_size,
                                               layer_num,
