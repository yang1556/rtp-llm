...
inline PyWrappedModel::PyWrappedModel(const GptModelInitParams& params,
    ...
    if (enable_cuda_graph_) {
        ...
        graph_runner_ = new GraphBaseRunner(
...
#if USING_ROCM
        // Register NCCL comm with collective_torch for HIP Graph capture (decoupled from graph runner).
        {
            ...
            if (nccl_comm != nullptr && init_params.tp_size > 1) {
                py::gil_scoped_acquire gil;
                try {
                    py::module_ ct = py::module_::import("rtp_llm.models_py.distributed.collective_torch");
                    ct.attr("set_graph_capture_nccl_comm")(reinterpret_cast<int64_t>(nccl_comm),
                                                           static_cast<int>(init_params.tp_size),
                                                           static_cast<int>(init_params.tp_rank));
                    RTP_LLM_LOG_INFO("Registered NCCL comm for graph capture (rank=%zu, world_size=%zu)",
                                     init_params.tp_rank,
                                     init_params.tp_size);
