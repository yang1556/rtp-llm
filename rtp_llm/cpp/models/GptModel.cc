...
GptModel::splitInputsIntoMicroBatches(const GptModelInputs& inputs, const MicroBatchPlan& micro_batch_plan) {
    auto result = rtp_llm::splitInputsIntoMicroBatches(inputs, micro_batch_plan, device_);
    // Preserve original behavior: when micro batch is disabled, allocate and hold fake_hidden for the second (fake)
    // input.
    if (!micro_batch_plan.enable && result.first.size() > 1) {
        ...
        buffer_holder_.hold(fake_hidden);
    }
    return result;
}
