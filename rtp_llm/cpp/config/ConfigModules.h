...
struct CacheStoreConfig {
    ...
    int64_t p2p_read_steal_before_deadline_ms   = 250;  // Decode read：在此距 deadline 时从 recv store steal，阻止新 transfer 匹配
    int64_t p2p_read_return_before_deadline_ms  = 100;  // Decode read 与 Prefill send：transfer 层 deadline / worker 须在 D 前该毫秒数完成（与对端 recv/send 对齐）
    int64_t p2p_transfer_not_done_resource_hold_ms = 10 * 1000;  // Scheduler：TRANSFER_NOT_DONE 后延迟 done 以保留显存安全窗口
