...
GraphBaseRunner::GraphBaseRunner(const DeviceInitParams& params,
...
#if USING_ROCM
    nccl_capture_ctx_(std::make_shared<graph_runner::GraphNcclCaptureContext>()),
#endif
    ...
    forward_event_(graph_runner::event_device_type()) {
...
graph_runner::GraphNcclCaptureContext* GraphBaseRunner::getNcclCaptureContext() {
#if USING_ROCM
    return nccl_capture_ctx_.get();
#else
    return nullptr;
#endif
}
...
void GraphBaseRunner::prepareInputs(PyModelInputs& inputs) {
    ...
    if (!is_prefill_graph_mode_) {
        ...
        graph_runner::memcpy_async(inputs.attention_inputs.sequence_lengths_plus_1_d,
                                   py_model_inputs_.attention_inputs.sequence_lengths_plus_1_d,
                                   state_.current_batch_size * sizeof(int));
        graph_runner::memcpy_async(inputs.attention_inputs.decode_cu_seqlens_d,
        ...
        graph_runner::memcpy_async(inputs.input_ids, py_model_inputs_.input_ids, state_.current_seq_len * sizeof(int));

        graph_runner::memcpy_async(inputs.attention_inputs.padding_offset,
        ...
        if (inputs.bert_embedding_inputs.position_encoding.numel() > 0) {
            graph_runner::memcpy_async(inputs.bert_embedding_inputs.combo_position_ids,
                                       py_model_inputs_.bert_embedding_inputs.combo_position_ids,
                                       state_.current_seq_len * sizeof(int));
            graph_runner::memcpy_async(inputs.bert_embedding_inputs.combo_tokens_type_ids,
    ...
    graph_runner::memcpy_async(inputs.attention_inputs.input_lengths,
                               py_model_inputs_.attention_inputs.input_lengths,
                               state_.current_batch_size * sizeof(int));

    graph_runner::memcpy_async(inputs.attention_inputs.cu_seqlens,
                               py_model_inputs_.attention_inputs.cu_seqlens,
                               (state_.current_batch_size + 1) * sizeof(int));

    graph_runner::memcpy_async(inputs.attention_inputs.cu_kv_seqlens,
    ...
    if (inputs.attention_inputs.kv_cache_layer_to_group.defined()
        ...
        graph_runner::memcpy_async(inputs.attention_inputs.kv_cache_layer_to_group,
...
PyModelOutputs GraphBaseRunner::forward(PyModelInputs& inputs) {
    ...
    graph_runner::record_forward_event(forward_event_);
    graph_runner::synchronize_forward_stream();
    return outputs;
}
...
void GraphBaseRunner::tryGetRealGraphDecodeBatchSize(PyModelInputs& inputs) {
    ...
    RTP_LLM_CHECK_WITH_INFO(it != capture_range_.end(),
                            "batch size %d exceeds captured max %d",
                            state_.current_batch_size,
                            capture_range_.empty() ? -1 : capture_range_.back());
...
void GraphBaseRunner::initCaptureAttentionInputs(PyModelInputs& inputs, int max_bs, int num_tokens_per_bs) {
    ...
    inputs.attention_inputs.sequence_lengths.fill_(max_seq_len_ - num_tokens_per_bs_ - 1);
    int kv_cols                                      = graph_runner::kv_block_cols(max_seq_len_, seq_size_per_block_);
    ...
    inputs.attention_inputs.sequence_lengths_plus_1_d =
        graph_runner::sequence_lengths_plus_one_tensor(int(max_bs_), options_device_int32_);
...
void GraphBaseRunner::initCapture() {
    if (enable_graph_) {
        if (graph_runner::should_skip_decode_capture(py_instance_, is_prefill_graph_mode_)) {
...
void GraphBaseRunner::captureOneGraphInstance(int key, const char* key_type) {
    ...
    graph_runner::device_synchronize();
    graph_runner::before_capture_stream(py_instance_, key, key_type);

    graph_runner::with_capture_stream([&]() {
        ...
        if (enable_graph_debug_mode_) {
            ...
            output_dot_filename = std::string(graph_runner::debug_file_prefix()) + std::to_string(num_tokens_per_bs_)
                                  + "_" + key_type_str + "_" + std::to_string(key) + "_visualization.dot";
        }

        graph_runner::enter_graph_capture(getNcclCaptureContext());
        graph.capture_begin();
        ...
        graph.capture_end();
        graph_runner::exit_graph_capture(getNcclCaptureContext());
