...
class SparseMlaFp8CPOp(SparseMlaFp8Op):
    ...
    def forward(
        ...
        # All-gather KV so each rank has full context
        all_compressed_kv = all_gather(compressed_kv, group=Group.TP)
        all_k_pe = all_gather(k_pe, group=Group.TP)
        # all_*: [total_kv_len * cp_size, dim]
        all_compressed_kv = all_compressed_kv.reshape(-1, self.kv_lora_rank)
        all_k_pe = all_k_pe.reshape(-1, self.qk_rope_head_dim)

        # Restore to logical order (valid tokens only)
        all_kv_compressed_restored = all_compressed_kv[self.kv_restore_unpad_indices]
        all_k_pe_restored = all_k_pe[self.kv_restore_unpad_indices]
        assert kv_cache is not None, "kv_cache is required for CP path"
        assert (
            self.kv_cache_write_op is not None
        ), "kv_cache_write_op must be set by SparseMlaCpImpl"

        # Write via the same kv_cache_write_op as non-CP (flashmla_sparse_impl line 508)
        ck = all_kv_compressed_restored
        kpe = all_k_pe_restored

        # Use existing slot_mapping from fill_params(attn_inputs); in CP prefill attn_inputs
        # describe the full context, so slot_mapping length and order match restored KV (N).
        self.kv_cache_write_op.forward(ck, kpe, kv_cache, self.mla_params)
...
class SparseMlaCpImpl(SparseMlaImpl):
    ...
    def __init__(
        ...
        self.fmha_impl = SparseMlaFp8CPOp(
