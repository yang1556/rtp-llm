...
class RoundRobinSparseMlaFp8CPOp(SparseMlaFp8Op):
    ...
    def __init__(
        ...
        if self.prefill_cp_size <= 1:
            raise ValueError(
                "RoundRobinSparseMlaFp8CPOp requires tp_size > 1; "
                f"got tp_size={self.prefill_cp_size}."
            )
    ...
    def _alloc_local_shard_rows(self, kv_cache: KVCache) -> torch.Tensor:
        kv_dim_bytes = kv_cache.kv_cache_base.size(-1)
        expected_shape = (self._total_local_kv, kv_dim_bytes)
        if (
            self._local_shard_rows is None
            or self._local_shard_rows.shape != expected_shape
        ):
            self._local_shard_rows = torch.empty(
                expected_shape,
                dtype=kv_cache.kv_cache_base.dtype,
                device=self.device,
            )
        return self._local_shard_rows

    def _get_padded_topk_buffer(
        self, local_tokens: int, topk: torch.Tensor
    ) -> torch.Tensor:
        expected_shape = (local_tokens, *topk.shape[1:])
        if (
            self._prefix_padded_topk_buffer is None
            or self._prefix_padded_topk_buffer.shape != expected_shape
        ):
            self._prefix_padded_topk_buffer = torch.empty(
                expected_shape,
                dtype=topk.dtype,
                device=self.device,
            )
        self._prefix_padded_topk_buffer.fill_(-1)
        return self._prefix_padded_topk_buffer
    ...
    def _materialize_workspace_from_rows(
        self,
        restored_rows: torch.Tensor,
        kv_cache: KVCache,
    ) -> torch.Tensor:
        workspace_cache = self._alloc_workspace_kv_cache(kv_cache)
        workspace_rows = workspace_cache.view(-1, workspace_cache.size(-1))
        workspace_rows[self._ws_slot_mapping] = restored_rows
        return workspace_cache

    def _normalize_local_topk_rows(
        self,
        q: torch.Tensor,
        topk: torch.Tensor,
        path_name: str,
        ...
        if topk.shape[0] == valid_local_tokens:
            return topk
        if topk.shape[0] == local_tokens:
            return torch.index_select(topk, 0, self.total_local_ids)
        raise ValueError(
            f"[RoundRobin {path_name}] topk rows must be either valid local rows "
            f"(aligned with total_local_ids) or fully padded local rows "
            f"(aligned with q). Got topk.shape[0]={topk.shape[0]}, "
    ...
    def _build_workspace_from_sharded_cache(self, kv_cache: KVCache) -> torch.Tensor:
        local_rows = self._alloc_local_shard_rows(kv_cache)
        local_rows.zero_()
        if self._local_kv_pack_dst_rows.numel() > 0:
            kv_cache_rows = kv_cache.kv_cache_base.view(
        ...
        return self._materialize_workspace_from_rows(restored_rows, kv_cache)
    ...
    def _forward_workspace_from_cache(
        ...
        normalized_topk = self._normalize_local_topk_rows(q, topk, "workspace")

        # Attention — valid local q rows attend to full global KV workspace
    ...
    def _forward_prefix_cache_ag_q(
        ...
        if topk.shape[0] not in (valid_local_tokens, local_tokens):
            raise ValueError(
        ...
        if topk.shape[0] < local_tokens:
            padded_topk = self._get_padded_topk_buffer(local_tokens, topk)
