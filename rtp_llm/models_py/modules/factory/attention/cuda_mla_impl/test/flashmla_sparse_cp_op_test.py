...
class SparseMlaFp8CPOpTest(TestCase):
    ...
    def test_cp_op_forward_with_prefix_lens(self):
        """
        With tp_size=1 and non-zero prefix_lengths, CP path all_gather is identity.
        Prefix tokens are already in KV cache; only new tokens are written.
        Run CP op and verify output shape and that it does not crash.
        """
        from rtp_llm.models_py.modules.factory.attention.cuda_mla_impl.flashmla_sparse_cp_impl import (
            SparseMlaFp8CPOp
        )
