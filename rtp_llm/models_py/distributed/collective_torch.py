...
def set_graph_capture_nccl_comm(
    nccl_comm_handle: int, world_size: int, rank: int
) -> None:
    """Register the C++ ncclComm_t handle for graph capture mode.

    Called once during initialization (e.g. from PyWrappedModel). Capture mode
    is toggled separately via enter_graph_capture_mode() / exit_graph_capture_mode().

    Args:
        nccl_comm_handle: The ncclComm_t pointer from C++ (as int64)
        world_size: TP world size
        rank: TP rank
    """
    global _rccl_comm, _rccl_world_size, _rccl_rank

    lib = _get_rccl_lib()
    if lib is None:
        logging.warning("set_graph_capture_nccl_comm: RCCL library not available")
        return
    ...
    _rccl_rank = rank
    logging.info(
        f"Registered NCCL comm for graph capture (rank={rank}, world_size={world_size}, "
        f"handle=0x{nccl_comm_handle:x})"
    )


def enter_graph_capture_mode(
    nccl_comm_handle: int = 0, world_size: int = 0, rank: int = 0
) -> None:
    """Enter HIP Graph capture mode.

    If nccl_comm_handle is non-zero, registers the comm (same as set_graph_capture_nccl_comm)
    then sets capture mode. Otherwise uses the already-registered comm and only sets
    _in_graph_capture = True. Called from C++ GraphBaseRunner before graph.capture_begin().

    Args:
        nccl_comm_handle: Optional; ncclComm_t pointer from C++ (as int64). If 0, use existing.
        world_size: Optional; TP world size (used only if nccl_comm_handle != 0).
        rank: Optional; TP rank (used only if nccl_comm_handle != 0).
    """
    global _in_graph_capture

    if nccl_comm_handle != 0:
        set_graph_capture_nccl_comm(nccl_comm_handle, world_size, rank)
    _in_graph_capture = True
    logging.info("Entered HIP Graph capture mode")
