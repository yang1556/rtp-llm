...
def _test_all_reduce_collective(
    ...
    for group_type in [Group.DP_AND_TP, Group.DP, Group.TP]:
        ...
        logging.info(
            f"Rank {rank} process_group: {process_group}, group_ranks: {group_ranks}"
        )
...
def _test_broadcast_collective(
    ...
    for group_type in [Group.DP_AND_TP, Group.DP, Group.TP]:
        ...
        if len(group_ranks) >= 2 and rank in group_ranks:
            ...
            if rank == src_rank:
                tensor = torch.tensor(
                    [1.0, 2.0, 3.0], device=f"cuda:{parallelism_config.local_rank}"
                )
            else:
...
def _test_send_recv_collective(
    ...
    for group_type in [Group.DP_AND_TP, Group.DP, Group.TP]:
        ...
        if len(group_ranks) >= 2:
            ...
            if rank == src_rank:
                logging.info(
                    f"Rank {rank} sending to rank {dst_rank}, {group_type} {group_ranks}"
                )
                tensor = torch.tensor(
                    [1.0, 2.0, 3.0], device=f"cuda:{parallelism_config.local_rank}"
                )
                ...
                logging.info(
                    f"Rank {rank} receiving from rank {src_rank}, {group_type} {group_ranks}"
                )
                tensor = torch.zeros(3, device=f"cuda:{parallelism_config.local_rank}")
...
def _test_all_gather_collective(
    ...
    for group_type in [Group.DP_AND_TP, Group.DP, Group.TP]:
        ...
        # Only test if this rank is in the group
        if rank in group_ranks:
            tensor = torch.ones(2, device=f"cuda:{parallelism_config.local_rank}") * (
                rank + 1
            )
            ...
            expected_list = [
                torch.ones(2, device=f"cuda:{parallelism_config.local_rank}") * (r + 1)
...
def _test_all_collectives_worker(
    ...
    logging.info(
        "test world_size: %d, tp_size: %d, dp_size: %d", world_size, tp_size, dp_size
    )
    try:
        # Setup - initialize once for all tests
        parallelism_config = ParallelismConfig()
        # Same port layout as former NodeCommInfo: base_port = nccl_port + 11 => th_nccl_port = nccl_port
...
class TestCollectiveOperations(unittest.TestCase):
    ...
    def _run_test(
        self, worker_func, world_size: int, tp_size: int, dp_size: int, test_name: str
    ):
    ...
    def test_all_collectives_tp2_dp2(self):
        ...
        self._run_test(
            _test_all_collectives_worker,
            world_size=4,
            tp_size=2,
            dp_size=2,
            test_name="all_collectives_tp2_dp2",
        )
    ...
    def test_all_collectives_tp4_dp1(self):
        ...
        self._run_test(
            ...
            tp_size=4,
            dp_size=1,
            test_name="all_collectives_tp4_dp1",
        )
