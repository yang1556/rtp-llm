...
cp_gather_and_upconvert_fp8_kv_cache_kernel(const uint8_t* __restrict__ src_cache,  // [NUM_BLOCKS, BLOCK_SIZE, 656]
                                            __nv_bfloat16* __restrict__ dst_compressed_kv,  // [TOT_TOKENS, 512]
                                            __nv_bfloat16* __restrict__ dst_k_pe,           // [TOT_TOKENS, 64]
                                            const int32_t* __restrict__ block_table,
                                            const int32_t* __restrict__ seq_lens,
                                            const int32_t* __restrict__ workspace_starts,
                                            const int32_t block_size,
                                            const int32_t compressed_kv_dim,
                                            const int32_t k_pe_dim,
                                            const int64_t block_table_stride,
                                            const int64_t cache_block_stride,
                                            const int64_t cache_entry_stride,
                                            const int64_t dst_compressed_kv_stride,
                                            const int64_t dst_k_pe_stride) {
    ...
    dst_compressed_kv += static_cast<int64_t>(seq_start) * dst_compressed_kv_stride;
    dst_k_pe += static_cast<int64_t>(seq_start) * dst_k_pe_stride;
    ...
    for (int pid = split_start; pid < split_end; ++pid) {
        ...
        __nv_bfloat16* dst_compressed_kv_ptr = dst_compressed_kv + pid * dst_compressed_kv_stride;
        __nv_bfloat16* dst_k_pe_ptr          = dst_k_pe + pid * dst_k_pe_stride;
        ...
        if (tid < compressed_kv_dim) {
            const int     tile         = tid >> 7;
            const float   scale        = scales_ptr[tile];
            const uint8_t val          = no_pe_ptr[tid];
            dst_compressed_kv_ptr[tid] = fp8_uint8_to_bf16(val, scale);
        } else if (tid < compressed_kv_dim + k_pe_dim) {
            const int rope_idx     = tid - compressed_kv_dim;
            dst_k_pe_ptr[rope_idx] = rope_ptr[rope_idx];
        }
...
void cp_gather_and_upconvert_fp8_kv_cache(const torch::Tensor& src_cache,
    ...
    int32_t compressed_kv_dim = static_cast<int32_t>(dst_compressed_kv.size(1));
    int32_t k_pe_dim          = static_cast<int32_t>(dst_k_pe.size(1));
    ...
    TORCH_CHECK(src_cache.device() == dst_compressed_kv.device(),
                "src_cache and dst_compressed_kv must be on the same device");
    TORCH_CHECK(src_cache.device() == dst_k_pe.device(), "src_cache and dst_k_pe must be on the same device");
    ...
    TORCH_CHECK(dst_compressed_kv.dtype() == torch::kBFloat16, "dst_compressed_kv must be bfloat16");
    TORCH_CHECK(dst_k_pe.dtype() == torch::kBFloat16, "dst_k_pe must be bfloat16");
    TORCH_CHECK(compressed_kv_dim == 512, "compressed_kv_dim must be 512 for MLA");
    TORCH_CHECK(k_pe_dim == 64, "k_pe_dim must be 64 for MLA");

    int64_t block_table_stride       = block_table.stride(0);
    int64_t cache_block_stride       = src_cache.stride(0);
    int64_t cache_entry_stride       = src_cache.stride(1);
    int64_t dst_compressed_kv_stride = dst_compressed_kv.stride(0);
    int64_t dst_k_pe_stride          = dst_k_pe.stride(0);
    ...
    cp_gather_and_upconvert_fp8_kv_cache_kernel<<<grid, block, 0, stream>>>(
        ...
        reinterpret_cast<__nv_bfloat16*>(dst_compressed_kv.data_ptr()),
        reinterpret_cast<__nv_bfloat16*>(dst_k_pe.data_ptr()),
        block_table.data_ptr<int32_t>(),
