...
def compute_br(k, w, u, g):
    """Run recurrence with h0=0, return only h_final. No h[t] or v_new stored."""
    B, T, Hg, K = k.shape
    H = w.shape[2]
    V = u.shape[-1]
    ht = torch.empty(B, H, K, V, dtype=torch.float32, device=k.device)
...
def compute_M_total(k, w, g):
    B, T, Hg, K = k.shape
    H = w.shape[2]
    BK = min(64, K)
    M_total = torch.empty(B, H, K, K, dtype=torch.float32, device=k.device)
...
def chunk_gated_delta_rule_fwd_cp_scan(
    ...
    h_partial_final = compute_br(k=k, w=w, u=u, g=g)
    M_total = compute_M_total(k=k, w=w, g=g)
    ...
    h0_global = (
        ...
        else torch.zeros(B, H, K, V, dtype=torch.float32, device=k.device)
    )
    ...
    h0_true = torch.empty(B, H, K, V, dtype=torch.float32, device=k.device)
    ops = []
    if rank > 0:
    ...
    final_state = (
        true_final_local if (output_final_state and rank == world_size - 1) else None
    )
