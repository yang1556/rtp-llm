...
class GrammarStats:
    ...
    tree_traversal_time: List[float] = dataclasses.field(default_factory=list)
...
class XGrammarGrammar(BaseGrammarObject):
    ...
    def apply_vocab_mask(self, logits: torch.Tensor, vocab_mask: torch.Tensor) -> None:
        # Pure torch path to avoid external runtime dependencies.
        if logits.numel() == 0:
            return
        vocab_size = logits.shape[-1]
        idx = torch.arange(vocab_size, device=logits.device)
        word_idx = torch.div(idx, 32, rounding_mode="floor")
        bit_idx = idx % 32
        mask_words = vocab_mask.to(logits.device)[:, word_idx]
        allowed = ((mask_words >> bit_idx) & 1).bool()
